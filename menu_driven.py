...
def updateCapacity(cnm,capacity):
    pos,cname,cap=searchByName(cnm)
    if cname!=None:
        ans=input(f"Do you want to update {cap}")
        if ans=="y":
            lst=list((cname,cap))
            lst[1]=capacity
            x=tuple(lst)
...
while choice!=7:
    ...
    match choice:
        ...
        case 3:
            print("modify capacity of the course")
            cnm=input("enter the course name")
            capacity=input("enter the capacity")
            status=updateCapacity(cnm,capacity)
            if status==1:
               print("capacity updated successfully")
            elif status==2:
                print("course found but capacity not updated")
            else:
               print("not found")
